...
def load_data():
    ...
    #with open('../../data/train.csv', 'rb') as input_file
        #read = csv.reader(codecs.iterdecode(input_file, 'utf-8'), delimiter='\t')
...
def predict(text, bag_of_words, words_count, texts_count):
    ...
    for w, cnt in list(counts.items()):
        if len(w) <= 2:
            continue
        p_word = (bag_of_words.get(w, 0.0) + 1) / sum(bag_of_words.values())
        p_w_given_pos = (words_count['pos'].get(w, 0.0) + 1) / (sum(words_count['pos'].values()) + sum(bag_of_words.values()))
        p_w_given_neg = (words_count['neg'].get(w, 0.0) + 1) / (sum(words_count['neg'].values()) + sum(bag_of_words.values()))
