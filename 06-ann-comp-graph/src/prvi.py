...
class NeuralNetwork(ComputationalNode):
    ...
    def get_weights(self):
        ...
        for neuron in self.layers[0]:
            weights.append(neuron.multiply_node)
    ...
    def get_accuracy(self, testX, testY):
        ...
        for x, y in zip(testX, testY):
            p = 0.0 if self.predict(x)[0] < 0.5 else 1.0
            predicted.append(p)
        ...
        return accurate_count / len(testX)


if __name__ == '__main__':
    nn = NeuralNetwork()
    # TODO 12: konstruisati neuronsku mrezu za resavanje XOR problema
    nn.add(NeuralLayer(5, 15, 'sigmoid')) # proveri dali relu radi
    nn.add(NeuralLayer(15, 1, 'sigmoid')) # pazi koliko ulaza ima!!!
    ...
    history = nn.fit(X_train, y_train, learning_rate=0.1, momentum=0.9, nb_epochs=200, verbose=1)
    print(nn.get_accuracy(X_test, y_test))
